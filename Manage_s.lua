...
AddEventHandler("core:SaleStock", function(itemName, type, ID, somme)
    ...
    local sourceItem = xPlayer.getInventoryItem(itemName)
    local Sale = 0
	Sale = xPlayer.getAccount('black_money').money
    if type == "Depot" then 
            MySQL.Async.fetchAll('SELECT * FROM argent_entreprise WHERE id=@id', {['@id'] = ID}, function(result)
                for k, v in pairs(result) do
                    if Sale >= tonumber(somme) and tonumber(somme) > 0 then
    ...
    elseif type == "Retrait" then 
        MySQL.Async.fetchAll('SELECT * FROM argent_entreprise WHERE id=@id', {['@id'] = ID}, function(result)
            for k, v in pairs(result) do
                if v.argentsale >= tonumber(somme) then
...
ESX.RegisterServerCallback('core:ListeEmployes', function(source, CallBack, metier)
    ...
    MySQL.Async.fetchAll('SELECT * FROM users WHERE job = @job', {
        ...
        for k,v in pairs(Index) do
            table.insert(ListeEMP, {Nom = v.lastname, Prenom = v.firstname, DDN = v.datedenaissance, Identitfier = v.identifier, Grade = v.job_grade})
        end
        CallBack(ListeEMP)
    end)
end)
